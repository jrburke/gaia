...
function MessageListCard(domNode, mode, args) {
  ...
  listFunc.size = function() {
    ...
    // by messages_slice. However at that point, the headerCount may
  ...
  this.vScroll._dataBind = (function(model, node) {
    model.element = node;
...
MessageListCard.prototype = {
  ...
  messages_complete: function(newEmailCount) {
    ...
    // finish initialization now that data is available.
    ...
    // Search changes its size very often, so recalculate the height if
    // it changed. Otherwise could inadvertently chop off some results.
    if (this.mode === 'search') {
      this.vScroll.checkSize();
    }

    this.onNewMail(newEmailCount);

    // Load next chunk if waiting to send one.
    this.waitingOnChunkCount -= 1;
    if (this.waitingOnChunkCount < 0) {
      this.waitingOnChunkCount = 0;
    }
  ...
  _onVScrollStopped: function() {
    ...
    /*
    SCROLL TODO: consider shrinking the selection, but requires
    keeping some more numbers around for this to work
    if (shrinkLowIncl !== 0 ||
  ...
  _requestSnippets: function() {
    ...
    if (visibleIndices) {
      ...
      console.log('ASKING FOR SNIPPETS 2: ' + visibleIndices[0] + ' to ' +
              visibleIndices[1]);
   ...
   * size of the data available, indicated by headerCursor.headerCount.
   */
  loadNextChunk: function(highestIndex) {
    ...
    // Do not bother asking for more chunks yet until
    if (!this.vScroll.waitingForRecalculate &&
        !this.waitingOnChunkCount &&
        this.nextChunkHighestIndex) {
  ...
  messages_splice: function(index, howMany, addedItems,
    ...
    if (headerCursor.searchMode !== this.mode ||
        this._needVScrollData || !this.vScroll._inited) {
      return;
    }
  ...
  onClickMessage: function(messageNode, event) {
    ...
    // Skip nodes that are default/placeholder ones while

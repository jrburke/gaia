 ...
 * - { newMessageCount: 1, fromAddress, subject, messageId, accountName? }
...
function churnPerAccount(multipleAccounts, accountInfo, newByConv) {
  // No new messages?  Nothing to say!
  if (newByConv.size === 0) {
    return null;
  }
  // This account configured to not generate notifications?  Nothing to say!
  if (!accountInfo.notifyOnNew) {
    return null;
  }

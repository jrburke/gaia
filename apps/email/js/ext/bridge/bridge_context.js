define(function (require) {
  ...
  function NamedContext(name, type, bridgeContext) {
    logic.defineScope(this, type, { name: name, bridge: bridgeContext.bridge.name });
    this.name = name;
  ...
  BridgeContext.prototype = {
    createNamedContext: function (name, type) {
      var ctx = new NamedContext(name, type, this);
    ...
    cleanupNamedContext: function (name) {
      ...
      var ctx = this._namedContexts.get(name);
      this._namedContexts.delete(name);
      ctx.cleanup();
    },

    cleanupAll: function () {
      for (var namedContext of this._namedContext.values()) {

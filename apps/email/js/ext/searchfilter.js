...
define(['logic', './util', './allback', './syncbase', './date', './bodies/htmlchew', 'module', 'exports'], function (logic, $util, allback, $syncbase, $date, htmlchew, $module, exports) {
      ...
      function checkRecipList(list) {
        ...
        for (var i = 0; i < list.length; i++) {
          ...
          if (ret = matchRegexpOrString(phrase, recip.name, 0)) {
            ...
            if (matches.length < stopAfter) continue;
            return;
          }
          if (ret = matchRegexpOrString(phrase, recip.address, 0)) {
            ...
            if (matches.length >= stopAfter) return;
          }
        }
      }

      if (this.checkTo && header.to) checkRecipList(header.to);
      if (this.checkCc && header.cc && matches.length < stopAfter) checkRecipList(header.cc);
      if (this.checkBcc && header.bcc && matches.length < stopAfter) checkRecipList(header.bcc);
  ...
  function snippetMatchHelper(str, start, length, contextBefore, contextAfter, path) {
    if (contextBefore > start) contextBefore = start;
      ...
      for (var iBodyRep = 0; iBodyRep < body.bodyReps.length; iBodyRep++) {
        ...
        if (bodyType === 'plain') {
          for (var iRep = 0; iRep < bodyRep.length && matches.length < stopAfter; iRep += 2) {
            ...
            if (!matchQuotes && etype !== CT_AUTHORED_CONTENT) continue;
  ...
  function MessageFilterer(filters) {
    ...
    for (var i = 0; i < filters.length; i++) {
      ...
      if (filter.needsBody) this.bodiesNeeded = true;
    }
  }
  ...
  MessageFilterer.prototype = {
    ...
    testMessage: function (header, body) {
      ...
      try {
        for (var i = 0; i < filters.length; i++) {
          var filter = filters[i];
          if (filter.testMessage(header, body, matchObj)) matched = true;
        }
      } catch (ex) {
        console.error('filter exception', ex, '\n', ex.stack);
      }
      //console.log('   =>', matched, JSON.stringify(matchObj));
      if (matched) return matchObj;else return false;
    }
  };
  ...
  function SearchSlice(bridgeHandle, storage, phrase, whatToSearch) {
    ...
    if (phrase) {
      ...
      if (whatToSearch.author) filters.push(new AuthorFilter(phrase));
      if (whatToSearch.recipients) filters.push(new RecipientFilter(phrase, 1, true, true, true));
      if (whatToSearch.subject) filters.push(new SubjectFilter(phrase, 1, CONTEXT_CHARS_BEFORE, CONTEXT_CHARS_AFTER));
  ...
  SearchSlice.prototype = {
    ...
    set headerCount(val) {
      if (this._bridgeHandle) this._bridgeHandle.headerCount = val;
      return val;
    },
      ...
      var checkHandle = (function checkHandle(headers, resolvedGetBodyCalls) {
        ...
        for (i = 0; i < headers.length; i++) {
          var header = headers[i],
              body = resolvedGetBodyCalls ? resolvedGetBodyCalls[header.id][0] : null;
          this._headersChecked++;
          var matchObj = this.filterer.testMessage(header, body);
          if (matchObj) matchPairs.push({ header: header, matches: matchObj });
        }
        ...
        var willHave = this.headers.length + matchPairs.length,
            wantMore = !moreMessagesComing && willHave < this.desiredHeaders && canGetMore;
    ...
    reqNoteRanges: function (firstIndex, firstSuid, lastIndex, lastSuid) {
      ...
      if (lastIndex + 1 < this.headers.length) {
        ...
        this._bridgeHandle.sendSplice(lastIndex + 1, delCount, [],

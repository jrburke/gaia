define(function (require) {
  ...
  return TaskDefiner.defineComplexTask([{
    ...
    deriveMemoryStateFromPersistentState: function (persistentState, accountId, accountInfo, foldersTOC) {
      var inboxFolder = foldersTOC.getCanonicalFolderByType('inbox');
      return {
        memoryState: {
    ...
    'trigger_msg!*!add': function (persistentState, memoryState, triggerCtx, message) {
      if (!memoryState.inboxFolderId) {
        var inboxFolder = memoryState.foldersTOC.getCanonicalFolderByType('inbox');
      ...
      // - detect group change
      var curTaskGroupId = triggerCtx.rootTaskGroupId;
      ...
      if (message.date >= persistentState.pendingDate) {
        ...
        var summary = messageSummarize(message);

        var messageMap = persistentState.newByConv.get(convId);
        if (!messageMap) {
          messageMap = new Map();
    ...
    'trigger_msg!*!change': function (persistentState, memoryState, triggerCtx, messageId, preInfo, message, added, kept, removed) {
      ...
      // label)
      if (removed.has(memoryState.inboxFolderId) || message && message.flags.indexOf('\\Seen') !== -1) {

define(function (require) {
  ...
  const { accountIdFromFolderId, accountIdFromMessageId, convIdFromMessageId } = require('../id_conversions');
  ...
  return TaskDefiner.defineSimpleTask([{
    ...
    plan: co.wrap(function* (ctx, req) {
      ...
      var account = yield ctx.universe.acquireAccount(ctx, accountId);
      var draftFolderInfo = account.getFirstFolderWithType('localdrafts');
      ...
      var identity = account.identities[0];

      // -- Determine ConversationId and MessageId values
      var convId = undefined; // The conversation this message belongs to.
      var messageId = undefined; // The full messageId with conversationId baked in.
      ...
      var messageIdPiece = '~' + ctx.id;
      var umid = accountId + '.' + messageIdPiece;
      ...
      var guid = generateMessageIdHeaderValue();
      var date = NOW();
      ...
      // - Reply
        ...
        // - Forward
            ...
            // conversation.)
            var sourceMessageKey = [req.refMessageId, req.refMessageDate];
            var fromDb = yield ctx.beginMutate({
              messages: new Map([[sourceMessageKey, null]])
            });
            var sourceMessage = fromDb.messages.get(req.refMessageId);

define(['logic', '../date', '../syncbase', '../allback', '../db/mail_rep', 'activesync/codepages', 'activesync/codepages/AirSync', 'activesync/codepages/AirSyncBase', 'activesync/codepages/ItemEstimate', 'activesync/codepages/Email', 'activesync/codepages/ItemOperations', 'safe-base64', './activesync-streams', 'mimetypes', 'co', 'util', 'mime-streams', 'module', 'require', 'exports'], function (logic, $date, $sync, allback, mailRep, ASCP, $AirSync, $AirSyncBase, $ItemEstimate, $Email, $ItemOperations, safeBase64, activesyncStreams, mimetypes, co, util, mimeStreams, $module, require, exports) {
  'use strict';
  ...
  function lazyConnection(cbIndex, fn, failString) {
    return function lazyRun() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        require(['wbxml', 'addressparser', '../bodies/mailchew'], function (wbxml, addressparser, mailchew) {
          if (!$wbxml) {
            $wbxml = wbxml;
  ...
  function ActiveSyncFolderConn(account, storage) {
    ...
    logic.defineScope(this, 'ActiveSyncFolderConn', { folderId: storage.folderId,
      accountId: account.id });

    this.folderMeta = storage.folderMeta;

    if (!this.syncKey) this.syncKey = '0';
  }
  ActiveSyncFolderConn.prototype = {
    ...
    get filterType() {
      ...
      if (SYNC_RANGE_TO_FILTER_TYPE.hasOwnProperty(syncRange)) {
        ...
        if (accountFilterType) return accountFilterType;else return this.folderMeta.filterType;
      } else {
    ...
    _getSyncKey: lazyConnection(1, function asfc__getSyncKey(filterType, callback) {
      ...
      if (account.conn.currentVersion.lt('12.1')) w.tag(as.Class, 'Email');
      ...
      account.conn.postCommand(w, function (aError, aResponse) {
        ...
        e.addEventListener([as.Sync, as.Collections, as.Collection, as.SyncKey], function (node) {
    ...
    _enumerateFolderChanges: lazyConnection(0, function asfc__enumerateFolderChanges(callback, progress) {
      ...
      if (this._account._syncsInProgress++ === 0 && this._account._lastSyncKey === this.syncKey && this._account._lastSyncFilterType === this.filterType && this._account._lastSyncResponseWasEmpty) {
        ...
        if (this._account.conn.currentVersion.lt('12.1')) w.tag(as.Class, 'Email');
      ...
      this._account.conn.postCommand(w, function (aError, aResponse) {
        ...
        e.addEventListener(base.concat(as.Commands, [[as.Add, as.Change]]), function (node) {
          var id, guid, msg;
        ...
        e.addEventListener(base.concat(as.Commands, [[as.Delete, as.SoftDelete]]), function (node) {
          var guid;
        ...
        if (status === asEnum.Status.Success) {
          ...
          if (moreAvailable) folderConn._enumerateFolderChanges(callback, progress);
        ...
        if (!totalBytes) totalBytes = Math.max(1000000, bytesSoFar);
    ...
    _parseMessage: function asfc__parseMessage(node, isAdded, storage) {
      ...
      if (isAdded) {
        ...
        flagHeader = function (flag, state) {
          if (state) header.flags.push(flag);
        };
      } else {
        header = {
          flags: [],
          mergeInto: function (o) {
            ...
            for (var iter in Iterator(this.flags)) {
              ...
              if (flagstate[1]) {
                ...
                if (index !== -1) o.flags.splice(index, 1);
              }
            }
        ...
        switch (child.tag) {
          ...
          case em.Flag:
            for (var iter2 in Iterator(child.children)) {
              ...
              if (grandchild.tag === em.Status) flagHeader('\\Flagged', grandchild.children[0].textContent !== '0');
            }
            break;
          case asb.Body:
            // ActiveSync 12.0+
            for (var iter2 in Iterator(child.children)) {
              ...
              switch (grandchild.tag) {
                case asb.Type:
                  ...
                  if (type === asbEnum.Type.HTML) bodyType = 'html';else {
                    // I've seen a handful of extra-weird messages with body types
                    // that aren't plain or html. Let's assume they're plain, though.
                    if (type !== asbEnum.Type.PlainText) console.warn('A message had a strange body type:', type);
                    bodyType = 'plain';
                  }
                  break;
          ...
          case em.Attachments:
            ...
            for (var iter2 in Iterator(child.children)) {
              ...
              if (attachmentNode.tag !== asb.Attachment && attachmentNode.tag !== em.Attachment) continue;
              ...
              if (isInline) body.relatedParts.push(mailRep.makeAttachmentPart(attachment));else body.attachments.push(mailRep.makeAttachmentPart(attachment));
            }
    ...
    downloadBodies: function (headers, options, callback) {
      if (this._account.conn.currentVersion.lt('12.0')) return this._syncBodies(headers, callback);
    ...
    downloadBodyReps: lazyConnection(1, function (header, options, callback) {
      ...
      if (account.conn.currentVersion.lt('12.0')) return this._syncBodies([header], callback);
      ...
      var gotBody = function gotBody(bodyInfo) {
        if (!bodyInfo) return callback('unknown');
        ...
        var w = new $wbxml.Writer('1.3', 1, 'UTF-8');
        w.stag(io.ItemOperations).stag(io.Fetch).tag(io.Store, 'Mailbox').tag(as.CollectionId, folderConn.serverId).tag(as.ServerId, header.srvid).stag(io.Options)
        // Only get the AirSyncBase:Body element to minimize bandwidth.
        .stag(io.Schema).tag(asb.Body).etag().stag(asb.BodyPreference).tag(asb.Type, bodyType);

        if (truncationSize) w.tag(asb.TruncationSize, truncationSize);
        ...
        account.conn.postCommand(w, function (aError, aResponse) {
          ...
          e.addEventListener([io.ItemOperations, io.Response, io.Fetch, io.Properties, asb.Body, asb.Data], function (node) {
          ...
          if (status !== ioEnum.Status.Success) return callback('unknown');
    ...
    _updateBody: function (header, bodyInfo, bodyContent, snippetOnly, callback) {
      ...
      if (!snippetOnly) bodyRep.content = data.content;
    ...
    downloadMessageAttachments: lazyConnection('promise', co.wrap(function* (uid, partInfos, handlers) {
      // Send the request.
      var folderConn = this;
      ...
      e.addEventListener([io.ItemOperations, io.Response, io.Fetch], function (node) {
        var attachmentInfo = {};
        ...
        for (var iter in Iterator(node.children)) {
          ...
          if (attachmentInfo.part) {
            attachments[attachmentInfo.part] = partIndexToAttachmentInfo[attachmentInfo.partIndex] = attachmentInfo;
          }
        }
      });

      var xmlBytes = util.concatBuffers.apply(null, (yield mimeStreams.readAllChunks(partStream)));
      ...
      while (true) {
        ...
        if (!done) {
          ...
          bodies[attachmentIndex] = yield mimeStreams.readAttachmentStreamWithChunkFlushing(attachmentInfo.contentType, partStream.pipeThrough(new mimeStreams.BlobTransformStream()), function (file) {
            return handlers.flushFileWithIndex(attachmentIndex, file);
          });
        } else {
          // All done!
          return bodies;
  ...
  function ActiveSyncFolderSyncer(account, folderStorage) {
    ...
    logic.defineScope(this, 'ActiveSyncFolderSyncer', { accountId: account.id,
  ...
  ActiveSyncFolderSyncer.prototype = {
    ...
    onSyncCompleted: function ifs_onSyncCompleted(doneCallback, initialSync, err, bisectInfo, messagesSeen) {
      ...
      if (!err) storage.markSyncedToDawnOfTime();
      ...
      this._account.__checkpointSyncCompleted(function () {
        if (err) {
          ...
          storage.getMessagesInImapDateRange(0, null, $sync.INITIAL_FILL_SIZE, $sync.INITIAL_FILL_SIZE,
          // Don't trigger a refresh; we just synced.  Accordingly,
          // releaseMutex can be null.
          storage.onFetchDBHeaders.bind(storage, storage._curSyncSlice, false, doneCallback, null));
        } else {
          doneCallback(err);
        }
      });

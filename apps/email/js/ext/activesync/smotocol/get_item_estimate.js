define(function (require) {
  ...
  const { Tags: ie, Enums: ieEnum } = require('activesync/codepages/ItemEstimate');
   ...
   */
  function* getItemEstimate(conn, { folderSyncKey, folderServerId, filterType }) {

    var w = new $wbxml.Writer('1.3', 1, 'UTF-8');
    w.stag(ie.GetItemEstimate).stag(ie.Collections).stag(ie.Collection);

    if (conn.currentVersion.gte('14.0')) {
      w.tag(as.SyncKey, folderSyncKey).tag(ie.CollectionId, folderServerId).stag(as.Options).tag(as.FilterType, filterType).etag();
    } else if (conn.currentVersion.gte('12.0')) {
      w.tag(ie.CollectionId, folderServerId).tag(as.FilterType, filterType).tag(as.SyncKey, folderSyncKey);
    } else {
      w.tag(ie.Class, 'Email').tag(as.SyncKey, folderSyncKey).tag(ie.CollectionId, folderServerId).tag(as.FilterType, filterType);
    }

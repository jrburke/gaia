define(function (require) {
   ...
   */
  function* enumerateFolderChanges(conn, { folderSyncKey, folderServerId, filterType, issueIds, emitter }) {
    ...
    w.tag(as.SyncKey, folderSyncKey).tag(as.CollectionId, folderServerId).tag(as.GetChanges).stag(as.Options).tag(as.FilterType, filterType);
    ...
    e.addEventListener(base.concat(as.Commands, as.Add), function (node) {
      var messageServerId = undefined,
          nodeToParse = undefined;
      ...
      if (nodeToParse && messageServerId) {
        try {
          var message = parseFullMessage(nodeToParse, issueIds());
          addCount++;
    ...
    if (status === asEnum.Status.Success) {
      logic(conn, 'syncComplete', { added: addCount, changed: changeCount, removed: removeCount });

define(function (require) {
   ...
   */
  function* inferFilterType(conn, { folderServerId, desiredMessageCount }) {
    const Type = $AirSync.Enums.FilterType;

    // -- Get a 2-week syncKey
    var filterType = Type.TwoWeeksBack;
    var { syncKey } = yield* getFolderSyncKey(conn, { folderServerId, filterType });

    // -- Get the item estimate for that 2-week syncKey
    var { estimate } = yield* getItemEstimate(conn, { folderSyncKey: syncKey, folderServerId, filterType });

    // -- Math!
    var messagesPerDay = estimate / 14; // Two weeks. Twoooo weeeeeeks.
    var desiredFilterType = undefined;
    ...
    if (estimate < 0) {
      ...
      syncKey = (yield* getFolderSyncKey(conn, { folderServerId, filterType })).syncKey;
      estimate = (yield* getItemEstimate(conn, { folderSyncKey: syncKey, folderServerId, filterType })).estimate;

...
define(function (require, exports) {
  ...
  function makeBodyPartsFromTextAndHTML(textMsg, htmlMsg) {
    ...
    bodyReps.push(makeBodyPart({
      ...
      isDownloaded: true,
      _partInfo: {},
      contentBlob: new Blob([JSON.stringify([0x1, textMsg])], { type: 'application/json' })
    }));
   ...
   */
  exports.generateReplyParts = function* (reps, authorPair, msgDate, identity, refGuid) {
    ...
    for (var i = 0; i < reps.length; i++) {
      var repType = reps[i].type;
      var repBlob = reps[i].contentBlob;
    ...
    if (identity.signature && identity.signatureEnabled) {
      ...
      if (htmlMsg) {
        htmlMsg += $htmlchew.wrapTextIntoSafeHTMLString(identity.signature, 'pre', false, ['class', 'moz-signature', 'cols', '72']);
      } else {
        textMsg += '\n\n-- \n' + identity.signature;
      }
    }
  ...
  exports.generateForwardParts = function* (sourceMessage, identity) {
    ...
    // : from
    textMsg += l10n_forward_header_labels['from'] + ': ' + $util.formatAddresses([sourceMessage.author]) + '\n';
    // : reply-to
    if (sourceMessage.replyTo) {
      textMsg += l10n_forward_header_labels['replyTo'] + ': ' + $util.formatAddresses([sourceMessage.replyTo]) + '\n';
    }
    // : organization
    // : to
    if (sourceMessage.to && sourceMessage.to.length) {
      textMsg += l10n_forward_header_labels['to'] + ': ' + $util.formatAddresses(sourceMessage.to) + '\n';
    }
    // : cc
    if (sourceMessage.cc && sourceMessage.cc.length) {
      textMsg += l10n_forward_header_labels['cc'] + ': ' + $util.formatAddresses(sourceMessage.cc) + '\n';
    }
    ...
    var reps = sourceMessage.bodyReps;
    for (var i = 0; i < reps.length; i++) {
      var repType = reps[i].type;
      var repBlob = reps[i].contentBlob;
  ...
  exports.processMessageContent = function processMessageContent(content, type, isDownloaded, generateSnippet) {
    ...
    switch (type) {
      case 'plain':
        ...
        contentBlob = new Blob([JSON.stringify(parsedContent)], { type: 'application/json' });
        break;
      case 'html':
        if (generateSnippet) {
          try {

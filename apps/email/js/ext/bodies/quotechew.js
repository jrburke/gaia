...
exports.quoteProcessTextBody = function quoteProcessTextBody(fullBodyText) {
  ...
  function countNewlinesInRegion(string, startIndex, endIndex) {
    ...
    for (;;) {
      idx = string.indexOf(NEWLINE, idx + 1);
      if (idx === -1 || idx >= endIndex) return count;
      count++;
    }
    return null;
  }
  ...
  exports.quoteProcessTextBody = function quoteProcessTextBody(fullBodyText) {
    ...
    function countQuoteDepthAndNormalize() {
      ...
      if (lastStartOffset) line = line.substring(lastStartOffset);
      return count;
    }
    ...
    function lookBackwardsForBoilerplate(chunk) {
      ...
      function pushBoilerplate(contentType, merge) {
        ...
        if (!merge) {
          contentRep.splice(insertAt, 0, (ate & 0xff) << 8 | contentType, boilerChunk);
        } else {
      ...
      for (idxLineStart = chunk.lastIndexOf('\n') + 1, idxLineEnd = chunk.length; idxLineEnd > 0 && scanLinesLeft; idxLineEnd = idxLineStart - 1, idxLineStart = chunk.lastIndexOf('\n', idxLineEnd - 1) + 1, scanLinesLeft--) {
        ...
        if (!line.length || line.length === 1 && line.charCodeAt(0) === CHARCODE_NBSP) continue;
  ...
  function pushContent(considerForBoilerplate, upToPoint, forcePostLine) {
    ...
    function pushContent(considerForBoilerplate, upToPoint, forcePostLine) {
      if (idxRegionStart === null) {
        if (atePreLines) {
          // decrement atePreLines if we are not the first chunk because then we get
          // an implicit/free newline.
          if (contentRep.length) atePreLines--;
          contentRep.push((atePreLines & 0xff) << 8 | CT_AUTHORED_CONTENT);
          contentRep.push('');
        }
      } else {
        if (upToPoint === undefined) upToPoint = idxLineStart;
        ...
        contentRep.push((atePreLines & 0xff) << 8 | (atePostLines & 0xff) << 16 | CT_AUTHORED_CONTENT);
        ...
        if (considerForBoilerplate) {
          ...
          if (chunk.length !== newChunk.length) {
            ...
            if (atePostLines) {
              ...
              contentRep[iLastMeta] = (atePostLines & 0xff) << 16 | contentRep[iLastMeta];
              contentRep[iChunk - 1] = (atePreLines & 0xff) << 8 | CT_AUTHORED_CONTENT;
            }
            ...
            if (!newChunk.length) {
              if (atePreLines) {
                ...
                bpAte += atePreLines;
                contentRep[iChunk + 1] = (bpAte & 0xff) << 8 | contentRep[iChunk + 1] & 0xffff00ff;
              }
    ...
    function pushQuote(newQuoteDepth) {
      ...
      contentRep.push((atePostLines & 0xff) << 24 | (ateQuoteLines & 0xff) << 16 | inQuoteDepth - 1 << 8 | CT_QUOTED_REPLY);
      ...
      if (line.charCodeAt(0) === CHARCODE_GT) {
        ...
        if (!inQuoteDepth) {
          ...
          if (lastNonWhitespaceLine && RE_WROTE_LINE.test(lastNonWhitespaceLine)) {
            ...
            if (idxLastNonWhitespaceLineEnd === null) idxRegionStart = null;

            var leadin = lastNonWhitespaceLine;
            pushContent(!generatedQuoteBlock, upToPoint);
            var leadinNewlines = 0;
            if (upToPoint + 1 !== idxLineStart) leadinNewlines = countNewlinesInRegion(fullBodyText, upToPoint + 1, idxLineStart);
        ...
        if (quoteRunLines.length || line.length) quoteRunLines.push(line);else ateQuoteLines++;
      } else {
        if (inQuoteDepth) {
          pushQuote(0);
          idxLastNonWhitespaceLineEnd = null;
    ...
    for (var i = 0; i < rep.length; i += 2) {
      ...
      switch (etype) {
        case CT_AUTHORED_CONTENT:
          ...
          if (block.length < desiredLength) return block.trim().replace(RE_NORMALIZE_WHITESPACE, ' ');
          // try and truncate on a whitespace boundary
          var idxPrevSpace = block.lastIndexOf(' ', desiredLength);
          if (desiredLength - idxPrevSpace < MAX_WORD_SHRINK) return block.substring(0, idxPrevSpace).trim().replace(RE_NORMALIZE_WHITESPACE, ' ');
...
function expandQuotedPrefix(s, depth) {
  ...
  function expandQuoted(s, depth) {
    ...
    return s.replace(RE_NEWLINE, function (m, idx) {
      if (s.charCodeAt(idx + 1) === CHARCODE_NEWLINE) return nows;else return ws;
    });
  }

define(function (require) {
  ...
  function CronSyncSupport({ universe, db, accountManager }) {
    ...
    this.sendCronSync = router.registerSimple('cronsync', function (data) {
      var args = data.args;
      logic(_this, 'message', { cmd: data.cmd });
      switch (data.cmd) {
        case 'alarm':
          _this.onAlarm.apply(_this, args);
          break;
        case 'syncEnsured':
          _this.onSyncEnsured.apply(_this, args);
          break;
        default:
          break;
      }
  ...
  CronSyncSupport.prototype = {
    ...
    ensureSync: function (why) {
      ...
      this._ensureSyncPromise = new Promise(function (resolve) {
        // No error pathway for the bridge hop, so just tracking resolve.
        _this2._ensureSyncResolve = resolve;
      });

      var syncData = {};

      for (var accountDef of this._accountManager.getAllAccountDefs()) {
        ...
        var interval = accountDef.syncInterval,
    ...
    cronsyncAccount: function ({ accountId, logTimestamp }) {
      ...
      var cronsyncLogEntry = {
        accountId,
        startTS: null,
        endTS: null,
        status: null
      };
      var cronsyncLogWrapped = {
      ...
      var foldersTOC = this._accountManager.accountFoldersTOCs.get(accountId);
      if (!foldersTOC) {
      ...
      var inboxFolderId = foldersTOC.getCanonicalFolderByType('inbox').id;
      this._universe.syncRefreshFolder(inboxFolderId, 'cronsync').then(function () {
        _this3._activelyCronSyncingAccounts.delete(accountId);
        ...
        _this3._db.updateBoundedLogs([cronsyncLogWrapped]);
      });
     ...
     */
    onAlarm: function (syncAccountIds, interval, wakelockId, accountIdsWithNotifications) {
      var _this4 = this;

      logic(this, 'alarmFired', { syncAccountIds, interval, wakelockId });

      // This is the timestamp we'll use with our log entry.
      var logTimestamp = NOW();

      var wakelock = wrapMainThreadAcquiredWakelock({
        wakelockId,
      ...
      var cronsyncsIssued = 0;
      for (var accountId of syncAccountIds) {
        if (this.cronsyncAccount({ accountId, logTimestamp })) {
          cronsyncsIssued++;
        }
      }

      var logConclusion = function (result) {
        cronsyncLogEntry.endTS = NOW();
        cronsyncLogEntry.endOnline = _this4._universe.online;
        cronsyncLogEntry.result = result;
        _this4._db.updateBoundedLogs([cronsyncLogWrapped]);
      };
    ...
    _cronsyncVictoriousCompletion: function () {
      ...
      var wakelockOnEntry = this._activeWakeLock;
      var logConclusionOnEntry = this._activeCronSyncLogConclusion;
      this._activeWakeLock = null;
      this._activeCronSyncLogConclusion = null;
      ...
      var realCompletion = function () {
        ...
        if (_this5._activeWakeLock) {
          logic(_this5, 'cronSync:last-minute-handoff');
          logConclusionOnEntry('success-left-open');
          wakelockOnEntry.unlock();
          return;
        }
        logic(_this5, 'cronSync:end');
        _this5._universe.broadcastOverBridges('cronSyncComplete', {});
    ...
    _cronsyncImminentDoom: function () {
      ...
      this._universe.broadcastOverBridges('cronSyncEpicFail', { epicnessLevel: 'so epic' });
    },

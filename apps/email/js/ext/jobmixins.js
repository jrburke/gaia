...
define(['./worker-router', './util', './allback', './wakelocks', './date', './syncbase', './mailslice', './headerCounter', 'co', 'logic', 'exports', 'require'], function ($router, $util, $allback, $wakelocks, $date, $sync, $mailslice, $count, co, logic, exports, require) {
  ...
  exports.local_do_modtags = function (op, doneCallback, undo) {
    ...
    this._partitionAndAccessFoldersSequentially(op.messages, false, function perFolder(ignoredConn, storage, headers, namers, callWhenDone) {
      ...
      for (var iHeader = 0; iHeader < headers.length; iHeader++) {
        ...
        storage.updateMessageHeader(header.date, header.id, false, header, /* body hint */null, next);
      }
    }, function () {
  ...
  exports.local_do_move = function (op, doneCallback, targetFolderId) {
    ...
    var stateDelta = this._stateDelta,
        addWait = 0,
        self = this;
    if (!stateDelta.moveMap) stateDelta.moveMap = {};
    if (!stateDelta.serverIdMap) stateDelta.serverIdMap = {};
    if (!targetFolderId) targetFolderId = op.targetFolder;
    ...
    this._partitionAndAccessFoldersSequentially(op.messages, false, function perFolder(ignoredConn, sourceStorage, headers, namers, perFolderDone) {
      ...
      function gotBody_nowDelete(_body) {
        ...
        if (header.srvid) stateDelta.serverIdMap[header.suid] = header.srvid;
        ...
        sourceStorage.folderMeta.type === 'localdrafts' && targetStorage.folderMeta.type !== 'outbox' || sourceStorage.folderMeta.type === 'outbox' && targetStorage.folderMeta.type !== 'localdrafts') {
          if (op.type === 'move') {
      ...
      function deleted_nowAdd() {
        ...
        if (nukeServerIds) header.srvid = null;
  ...
  exports.do_download = function (op, callback) {
    co((function* () {
      ...
      var { folderConn, folderStorage } = yield new Promise(function (resolve, reject) {
        _this._accessFolderForMutation(folderId, true, function (folderConn, folderStorage) {
      ...
      var header = yield new Promise(function (resolve) {
        folderStorage.getMessageHeader(op.messageSuid, op.messageDate, resolve);
      });

      var uid = header.srvid;

      var bodyInfo = yield new Promise(function (resolve) {
      ...
      var bodyBlobs = yield folderConn.downloadMessageAttachments(uid, partsToDownload, {
        flushFileWithIndex: function (index, file) {
          return new Promise(function (resolve) {
            ...
            folderStorage.updateMessageBody(header, bodyInfo, { flushBecause: 'blobs' }, {}, function (_bodyInfo) {
              bodyInfo = _bodyInfo;
      ...
      yield new Promise(function (resolve) {
        folderStorage.updateMessageBody(header, bodyInfo, { flushBecause: 'blobs' }, { changeDetails: { attachments: op.attachmentIndices } }, resolve);
      });
  ...
  exports.check_download = function (op, callback) {
    co((function* () {
      ...
      var { folderConn, folderStorage } = yield new Promise(function (resolve, reject) {
        _this2._accessFolderForMutation(folderId, true, function (folderConn, folderStorage) {
          resolve({ folderConn, folderStorage });
        }, function () {
          reject('aborted-retry'); // deadConn
        }, 'download');
  ...
  exports.do_downloadBodies = function (op, callback) {
    ...
    'downloadBodies', true // require headers
    );
  };
  ...
  exports.do_sendOutboxMessages = function (op, callback) {
    ...
    this._accessFolderForMutation(outboxFolder.id, /* needConn = */false, function (nullFolderConn, folderStorage) {
      require(['jobs/outbox'], function ($outbox) {
        $outbox.sendNextAvailableOutboxMessage(account.compositeAccount || account, // Requires the main account.
        folderStorage, op.beforeMessage, op.emitNotifications, outboxNeedsFreshSync, wakeLock).then(function (result) {
    ...
    /* no conn => no deathback required */null, 'sendOutboxMessages');
  };
    ...
    if (!error) {
      ...
      if (this._stateDelta.serverIdMap) {
        ...
        for (var suid in deltaMap) {
          ...
          if (srvid === null) delete fullMap[suid];else fullMap[suid] = srvid;
        }
      }
    ...
    var gotNeededHeaders = function gotNeededHeaders(headers) {
      ...
      for (var i = 0; i < headers.length; i++) {
        ...
        if (header) {
          ...
          // us.
          if (!srvid) console.warn('Header', headers[i].suid, 'missing server id in job!');
        }

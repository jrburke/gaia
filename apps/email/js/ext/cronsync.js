  ...
  CronSync.prototype = {
    ...
    syncAccount: function (account) {
      ...
      return new Promise(function (resolve) {
        var scope = logic.subscope(_this3, { accountId: account.id });

        // - Skip syncing if we are offline or the account is disabled
        if (!_this3._universe.online || !account.enabled) {
          debug('syncAccount early exit: online: ' + _this3._universe.online + ', enabled: ' + account.enabled);
        ...
        var slice = makeHackedUpSlice(storage, function (newHeaders) {
          ...
          if (newHeaders.length) {
            ...
            } else if (_this3._universe.online) {
              logic(scope, 'syncAccountSnippets_begin');
              _this3._universe.downloadBodies(newHeaders.slice(0, syncbase.CRONSYNC_MAX_SNIPPETS_TO_FETCH_PER_ACCOUNT), {
        ...
        if (outboxFolder) {
          ...
          if (outboxStorage.getKnownMessageCount() > 0) {
            ...
            _this3._universe.sendOutboxMessages(account, {
              reason: 'syncAccount'
            }, function () {
        ...
        latch.then(function (latchResults) {
          ...
          _this3._universe.waitForAccountOps(account, function () {
    ...
    onAlarm: function (accountIds) {
      ...
      var done = function () {
        ...
        _this4._ensureSyncPromise.then(function () {
          if (syncResults.length) {
            accountsResults.updates = syncResults;
          }

          _this4._universe.__notifyStoppedCronSync(accountsResults);
          logic(_this4, 'syncAccounts_end', { accountsResults: accountsResults });
          logic(_this4, 'cronSync_end');
        });
      };
      ...
      Promise.all(targetAccounts.map(function (account) {
        return _this4.syncAccount(account).then(function (result) {
          if (result) {
            syncResults.push({
              id: account.id,

define(function (require) {
  ...
  return {
    ...
    deriveMemoryStateFromPersistentState: function (persistentState, accountId) {
      var markers = [];
      for (var [messageId, { order }] of persistentState.messageIdsToSend) {
    ...
    _planSend: co.wrap(function* (ctx, persistentState, memoryState, rawTask) {
      ...
      if (!memoryState.paused) {
        var marker = this._makeMarkerForMessage(rawTask.accountId, messageId, sendInfo.order);
        modifyTaskMarkers.set(marker.id, marker);
      }

      // Provide a result to the caller that lets them know
      var reportProblem = undefined;
      if (memoryState.paused) {
    ...
    _planAbort: co.wrap(function* (ctx, persistentState, memoryState, rawTask) {
      ...
      var markerId = this._markerIdForMessage(messageId);
    ...
    _planSetPaused: co.wrap(function* (ctx, persistentState, memoryState, rawTask) {
      ...
      var bePaused = memoryState.paused = rawTask.paused;
      var modifyTaskMarkers = new Map();
      if (bePaused) {
        ...
        // being sent.)
        for (var messageId of persistentState.messageIdsToSend.keys()) {
        ...
        for (var [messageId, { order }] of persistentState.messageIdsToSend) {
          var marker = this._makeMarkerForMessage(accountId, messageId, order);
          modifyTaskMarkers.set(marker.id, marker);
        }
      }
    ...
    execute: co.wrap(function* (ctx, persistentState, memoryState, marker) {
      ...
      var messageKey = [messageId, date];
      var messageInfo = (yield ctx.read({
      ...
      // -- Perform the send.
      var { error: sendError, badAddresses } = yield this.sendMessage(ctx, account, composer);
      ...
      var modifyConversations = new Map();
      if (sendError) {

define(function (require) {
   ...
   */
  const uniqueifyingSaveHelper = co.wrap(function* (ctx, blob, filename) {
    /* always register with the download manager */
    const register = true;

    var result = yield sendStorageMessage('save', [DEVICE_STORAGE_NAME, blob, filename, register]);

    var [initialSucesss] = result;
    if (initialSucesss) {
      return result;
    }
    ...
    const retryFilename = filename.substring(0, idxLastPeriod) + '-' + NOW() + filename.substring(idxLastPeriod);

    result = yield sendStorageMessage('save', [DEVICE_STORAGE_NAME, blob, retryFilename, register]);
  ...
  return {
    ...
    deriveMemoryStateFromPersistentState: function (persistentState, accountId) {
      var markers = [];
      for (var messageId of persistentState.keys()) {
    ...
    overlay_messages: function (persistentState, memoryState, messageId) {
      var pending = persistentState.get(messageId);
      // If nothing is pending, nothing can be active either.  Nothing to say.
      if (!pending) {
        return null;
      }
      var active = memoryState.activeDownloadsByMessageId.get(messageId);
      var overlay = new Map();
      for (var [relId, { blobs }] of pending.partDownloads.entries()) {
        var isActive = active && active.has(relId);
        var bytesDownloaded = blobs ? blobs.reduce(function (sum, blob) {
    ...
    plan: co.wrap(function* (ctx, persistentState, memoryState, rawTask) {
      ...
      var messageReq = yield ctx.mutateSingle('complexTaskStates', messageTaskKey);
      if (!messageReq) {
        messageReq = {
          messageDate,
      ...
      var newlyRequestedCount = 0;

      const messageInfo = yield ctx.readSingle('messages', [messageId, messageDate], messageId);

      for (var [relId, target] of requestedParts.entries()) {
    ...
    execute: co.wrap(function* (ctx, persistentState, memoryState, marker) {
      ...
      var messageInfo = yield ctx.readSingle('messages', [messageId, messageDate], messageId);
      ...
      // avoid later confusion with the copies our subtask loads from disk.
      {
        ...
        // matter to us.)
        var _messageReq = persistentState.get(messageId);
        for (var relId of _messageReq.partDownloads.keys()) {
      ...
      const chunkedPartStream = yield this.downloadParts(ctx, account, messageInfo, parts);
      ...
      var _loop = function* () {
        var { value, done: streamDone } = yield chunkedPartReader.read();
        if (streamDone) {
          return 'break';
        }

        var { relId, blobCount, blob, done } = value;
        if (!done) {
          // - Not done, do the append dance.
          yield ctx.spawnSimpleMutationSubtask({ namespace: 'complexTaskStates', id: messageTaskKey }, function (messageReq) {
          ...
          if (blobCount > 1) {
            yield ctx.spawnSimpleMutationSubtask({ namespace: 'complexTaskStates', id: messageTaskKey }, function (messageReq) {
              ...
              const singleBlob = new Blob(multiBlobs, { type: multiBlobs[0].type });
          ...
          yield ctx.spawnSubtask(co.wrap(function* (subctx) {
            ...
            if (partReq.target === 'cache') {
              ...
              var [saveSuccess, saveError, savedFilename, registered] = yield uniqueifyingSaveHelper(ctx, singleBlob, part.name);
              if (saveSuccess) {
                part.downloadState = 'saved';
                part.file = [DEVICE_STORAGE_NAME, savedFilename];
                console.log('saved attachment to', savedFilename, 'type:', singleBlob.type, 'registered:', registered);
                ...
                // perspective.
                console.warn('failed to save attachment to', part.name, 'type:', singleBlob.type);
      ...
      const messageReq = fromDb.complexTaskStates.get(messageTaskKey);
      var modifyComplexTaskStates = undefined;
      var modifyTaskMarkers = undefined;

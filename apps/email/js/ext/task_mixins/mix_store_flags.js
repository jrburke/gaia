define(function (require) {
  ...
  var MixStoreFlagsMixin = {
    ...
    deriveMemoryStateFromPersistentState: function (persistentState, accountId) {
      ...
      for (var umid of persistentState.umidChanges.keys()) {
        markers.push({
          type: this.name,
          id: this.name + ':' + umid,
          accountId: accountId,
    ...
    plan: co.wrap(function* (ctx, persistentState, memoryState, req) {
      ...
      for (var message of filteredMessages) {
        var actualChanges = normalizeAndApplyChanges(message.flags, req.add, req.remove);
        var { add: actuallyAdded, remove: actuallyRemoved } = actualChanges;
        ...
        if (actuallyAdded || actuallyRemoved) {
          ...
          var umid = message.umid;
          var markerId = this.name + ':' + umid;
          // - Unify with any outstanding request for this message
          if (umidChanges.has(umid)) {
            var mergedChanges = mergeChanges(umidChanges.get(umid), actualChanges);
    ...
    consult: function (askingCtx, persistentState, memoryState, argDict) {
      var { umid, value } = argDict;

      var { umidChanges } = persistentState;

      if (umidChanges.has(umid)) {
        var changes = umidChanges.get(umid);

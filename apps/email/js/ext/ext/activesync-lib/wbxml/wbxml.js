...
(function (root, factory) {
  ...
  StringTable.prototype = {
    get: function (offset) {
      if (offset in this.offsets) return this.strings[this.offsets[offset]];else {
        if (offset < 0) throw new ParseError('offset must be >= 0');

        var curr = 0;
        for (var i = 0; i < this.strings.length; i++) {
          // Add 1 to the current string's length here because we stripped a
          // null-terminator earlier.
          if (offset < curr + this.strings[i].length + 1) return this.strings[i].slice(offset - curr);
    ...
    for (var name in codepages) {
      ...
      if (page.Attrs) {
        for (var attrName in page.Attrs) {
          ...
          if (!('name' in attrData)) attrData.name = attrName;
  ...
  Element.prototype = {
    ...
    getAttribute: function (attr) {
      if (typeof attr === 'number') attr = this.ownerDocument._codepages.__attrdata__[attr].name;else if (!(attr in this._attrs) && this.namespace !== null && attr.indexOf(':') === -1) attr = this.namespaceName + ':' + attr;
    ...
    _getAttribute: function (pieces) {
      ...
      if (strValue) array.push(strValue);
    ...
    _addAttribute: function (attr) {
      if (typeof attr === 'string') {
        if (attr in this._attrs) throw new ParseError('attribute ' + attr + ' is repeated');
        ...
        if (name in this._attrs) throw new ParseError('attribute ' + name + ' is repeated');
  ...
  ProcessingInstruction.prototype = {
    ...
    get target() {
      if (typeof this.targetID === 'string') return this.targetID;else return this.ownerDocument._codepages.__attrdata__[this.targetID].name;
    },

    _setTarget: function (target) {
      this.targetID = target;
      if (typeof target === 'string') return this._data = [];else return this._data = [target];
    },
  ...
  Reader.prototype = {
    _get_uint8: function () {
      if (this._index === this._data.length) throw EndOfData;
    ...
    _getDocument: function () {
      ...
      var appendString = (function (s) {
        if (state === States.BODY) {
          if (!currentNode) currentNode = new Text(this, s);else currentNode.textContent += s;
        } else {
          // if (state === States.ATTRIBUTES || state === States.ATTRIBUTE_PI)
          currentAttr.push(s);
        }
      ...
      try {
        while (true) {
          ...
          if (tok === Tokens.SWITCH_PAGE) {
            ...
            if (state === States.BODY && depth === 0) throw new ParseError('unexpected ENTITY token');
            var e = this._get_mb_uint32();
            appendString('&#' + e + ';');
          } else if (tok === Tokens.STR_I) {
            if (state === States.BODY && depth === 0) throw new ParseError('unexpected STR_I token');
            appendString(this._decoder.decode(this._get_c_string()));
          } else if (tok === Tokens.PI) {
            if (state !== States.BODY) throw new ParseError('unexpected PI token');
            state = States.ATTRIBUTE_PI;

            if (currentNode) doc.push(currentNode);
            currentNode = new ProcessingInstruction(this);
          } else if (tok === Tokens.STR_T) {
            if (state === States.BODY && depth === 0) throw new ParseError('unexpected STR_T token');
            var r = this._get_mb_uint32();
            appendString(this.strings.get(r));
          } else if (tok === Tokens.OPAQUE) {
            if (state !== States.BODY) throw new ParseError('unexpected OPAQUE token');
            ...
            if (depth === 0) {
              if (foundRoot) throw new ParseError('multiple root nodes found');
              foundRoot = true;
            }
            ...
            if (!(tok & 0x80)) {
              ...
              if (state === States.ATTRIBUTE_PI) {
                if (currentAttr) throw new ParseError('unexpected attribute in PI');
    ...
    dump: function (indentation, header) {
      ...
      if (indentation === undefined) indentation = 2;
  ...
  function Writer(version, pid, charset, strings, dataType) {
    ...
    // state.
    if (dataType === 'blob') this._blobs = [];else this._blobs = null;
    ...
    if (typeof charset === 'string') {
      ...
      if (charsetNum === undefined) throw new Error('unknown charset ' + charset);
    }
  ...
  Writer.Attribute = function (name, value) {
    ...
    if (this.isValue && value !== undefined) throw new Error("Can't specify a value for attribute value constants");
    this.name = name;
    this.value = value;
  };
  ...
  Writer.Extension = function (subtype, index, data) {
    ...
    if (!info) throw new Error('Invalid WBXML Extension type');
    if (!info.validator(data)) throw new Error('Data for WBXML Extension does not match type');
    if (index !== 0 && index !== 1 && index !== 2) throw new Error('Invalid WBXML Extension index');
  ...
  Writer.prototype = {
    _write: function (tok) {
      ...
      if (this._pos === this._buffer.length - 1) {
        ...
        for (var i = 0; i < this._buffer.length; i++) buffer[i] = this._buffer[i];
    ...
    _write_mb_uint32: function (value) {
      ...
      for (var i = bytes.length - 1; i >= 0; i--) this._write(bytes[i]);
    },

    _write_bytes: function (bytes) {
      for (var i = 0; i < bytes.length; i++) this._write(bytes[i]);
    },
    ...
    _writeTag: function (tag, stag, attrs) {
      if (tag === undefined) throw new Error('unknown tag');

      var flags = 0x00;
      if (stag) flags += 0x40;
      if (attrs.length) flags += 0x80;
      ...
      if (tag instanceof Writer.StringTableRef) {
        ...
        if (!this._rootTagValue) this._rootTagValue = tag;
      }
    ...
    _writeAttr: function (attr) {
      if (!(attr instanceof Writer.Attribute)) throw new Error('Expected an Attribute object');
      if (attr.isValue) throw new Error("Can't use attribute value constants here");
    ...
    _writeText: function (value, inAttr) {
      if (Array.isArray(value)) {
        ...
        if (!value.isValue) throw new Error('Unexpected Attribute object');
        if (!inAttr) throw new Error("Can't use attribute value constants outside of " + "attributes");
    ...
    etag: function (tag) {
      if (this._tagStack.length === 0) throw new Error('Spurious etag() call!');
      var expectedTag = this._tagStack.pop();
      if (tag !== undefined && tag !== expectedTag) throw new Error('Closed the wrong tag');
    ...
    opaque: function (data) {
      ...
      if (data instanceof Blob) {
        ...
        // Array or Uint8Array
        this._write_mb_uint32(data.length);
        for (var i = 0; i < data.length; i++) this._write(data[i]);
      }
      return this;
    },
    ...
    get blob() {
      ...
      if (this._pos) useBlobs = useBlobs.concat([this.bytes]);
  ...
  EventParser.prototype = {
    ...
    _pathMatches: function (a, b) {
      return a.length === b.length && a.every(function (val, i) {
        if (b[i] === '*') return true;else if (Array.isArray(b[i])) {
      ...
      for (var iNode = 0; iNode < doclen; iNode++) {
        ...
        if (recording) {
          if (node.type === 'STAG') {
            node.type = 'TAG';
            node.children = [];
            if (recPath.length) recPath[recPath.length - 1].children.push(node);
            recPath.push(node);
          } else if (node.type === 'ETAG') {
            recPath.pop();
          } else {

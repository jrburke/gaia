...
define(function (require, exports, module) {
  ...
  var ALLOWED_TAGS = ['a', 'abbr', 'acronym', 'b', 'blockquote', 'code', 'em', 'i', 'li', 'ol', 'strong', 'ul'];
  ...
  HTMLSanitizer.prototype = {
    start: function (tag, attrs, unary) {
      ...
      if (this.pruneTags.indexOf(tag) !== -1) {
        if (!unary) this.isInsidePrunedTag++;
        return;
    ...
    chars: function (text) {
      if (this.isInsidePrunedTag || this.complete) return;
      if (this.isInsideStyleTag) {
    ...
    comment: function (comment) {
      if (this.isInsidePrunedTag) return;
      if (this.ignoreComment) return;
      this.output += '<!--' + comment + '-->';
    }
  };
        ...
        if (!stack.last() || !special[stack.last()]) {
          ...
          if (html.lastIndexOf("<!--", 0) == 0) {
            ...
            if (index >= 4) {
              ...
              if (handler.comment) handler.comment(html.substring(4, -1));
              html = '';
              chars = false;
            }
          ...
          if (chars) {
            ...
            if (handler.chars) {
              handler.chars(text);
              if (handler.complete) return this;
            }
          }
        } else {
          // specials: script or style
          var skipWork = false;
          html = html.replace(
          // we use "[^]" instead of "." because it matches newlines too
          new RegExp("^([^]*?)<\/" + stack.last() + "[^>]*>", "i"), function (all, text) {
            if (!skipWork) {
    ...
    _filterDeclarations: function (parent, decls, allowedStyles, fullText, textOut) {
      for (var i = 0; i < decls.length; i++) {
        ...
        if (allowedStyles.indexOf(decl.name) !== -1) {
          textOut.push(fullText.substring(decl.startTok.loc.start.idx,
          ...
          // parseAttribute.
          parent && parent.endTok === decl.endTok ? decl.endTok.loc.start.idx : decl.endTok.loc.end.idx + 1));
        }
      }
    },
  ...
  exports.unescapeHTMLEntities = function unescapeHTMLEntities(text) {
    return text.replace(entityRegExp, function (match, ref) {
      ...
      if (ref.charAt(0) === '#') {
        ...
        if (!reverseEntities) makeReverseEntities();

        if (reverseEntities.hasOwnProperty(ref)) converted = String.fromCharCode(reverseEntities[ref]);
      }
      return converted;
    });
  };

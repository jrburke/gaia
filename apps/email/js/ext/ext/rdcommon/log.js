...
define(['./deferred', './microtime', './extransform', 'exports'], function (Deferred, $microtime, $extransform, exports) {
  ...
  var ThingProto = exports.ThingProto = {
    ...
    toJSON: function () {
      ...
      if (this.__hardcodedFamily) o.family = this.__hardcodedFamily;
      return o;
    }
  };
  ...
  exports.__makeThing = function makeThing(type, humanName, digitalName, proto) {
    var thing;
    if (proto === undefined) proto = ThingProto;
  ...
  var LogProtoBase = {
    ...
    toJSON: function () {
      ...
      if (this.__latchedVars.length) {
        var latchedVars = this.__latchedVars,
            olv = {};
      ...
      if (this._named) jo.named = this._named;
      return jo;
    },
    __die: function () {
      this._died = $microtime.now();
      if (this.__FAB._onDeath) this.__FAB._onDeath(this);
    },
    __updateIdent: function (ident) {
      ...
      if (Array.isArray(ident)) {
        ...
        for (var i = 0; i < ident.length; i++) {
          ...
          if (typeof identBit !== "object" || identBit == null) normIdent.push(identBit);else normIdent.push(identBit._uniqueName);
        }
        ident = normIdent;
      }
      this._ident = ident;
  ...
  TestLogProtoBase.__die = function () {
    ...
    if (testActor) {
      ...
      if (testActor._lifecycleListener) testActor._lifecycleListener.call(null, 'dead', this.__instance, this);
    }
  };

  var DIED_EVENTNAME = '(died)',
      DIED_EXP = [DIED_EVENTNAME];
  ...
  var TestActorProtoBase = {
    ...
    __attachToLogger: function (logger) {
      logger._actor = this;
      this._logger = logger;
      if (this._lifecycleListener) this._lifecycleListener.call(null, 'attach', logger.__instance, logger);
    },
    ...
    __prepForTestStep: function (testRuntimeContext) {
      if (!this._logger) testRuntimeContext.reportPendingActor(this);
      // we should have no expectations going into a test step.
      if (this._activeForTestStep) this.__resetExpectations();
      ...
      if (this._logger) this._iEntry = this._logger._entries.length;
    },
    ...
    __waitForExpectations: function () {
      if (this._expectNothing && (this._expectations.length || this._iExpectation)) return false;
      ...
      // time.)
      if (!this._expectationsMetSoFar) return false;
      if (this._iExpectation >= this._expectations.length && (this._expectDeath ? this._logger && this._logger._died : true)) {
        this._resolved = true;
    ...
    __resetExpectations: function () {
      if (this.__stepCleanup) this.__stepCleanup();
    ...
    __failUnmetExpectations: function () {
      ...
      if (this._expectDeath && !this._logger._died) this._logger.__failedExpectation(DIED_EXP);
    },
    ...
    __loggerFired: function () {
      ...
      if (this._unorderedSetMode) {
        ...
        while (this._iExpectation < this._expectations.length && this._iEntry < entries.length) {
          entry = entries[this._iEntry++];
          // ignore meta-entries (which are prefixed with a '!')
          if (entry[0][0] === "!") continue;
          // ignore ignored entries
          if (this._ignore && this._ignore.hasOwnProperty(entry[0])) continue;

          // - try all the expectations for a match
          var foundMatch = false;
          for (var iExp = this._iExpectation; iExp < this._expectations.length; iExp++) {
            expy = this._expectations[iExp];

            // - on matches, reorder the expectation and bump our pointer
            if (expy[0] === entry[0] && this['_verify_' + expy[0]](expy, entry)) {
          ...
          if (!foundMatch) {
            ...
            if (this._deferred) this._deferred.reject([this.__defName, expy, entry]);
          }
        }

        // - generate an unexpected failure if we ran out of expectations
        if (this._iExpectation === this._expectations.length && entries.length > this._iEntry) {
        ...
        else if (this._iExpectation >= this._expectations.length && this._deferred && (this._expectDeath ? this._logger && this._logger._died : true)) {
            this._resolved = true;
      ...
      //  come in.
      if (this._activeForTestStep && (this._expectations.length && this._iExpectation === this._expectations.length && entries.length > this._iEntry || !this._expectations.length && this._expectNothing)) {
        // Only get upset if this is not an ignored event.
        if (!this._ignore || !this._ignore.hasOwnProperty(entries[this._iEntry][0])) {
          this._expectationsMetSoFar = false;
          this._logger.__unexpectedEntry(this._iEntry, entries[this._iEntry]);
        }
      ...
      if (this._iExpectation >= this._expectations.length && this._deferred && (this._expectDeath ? this._logger && this._logger._died : true)) {
        this._resolved = true;
        this._deferred.resolve();
      }
    }
  ...
  function simplifyInsaneObjects(obj, dtype, curDepth) {
    ...
    if (dtype) {
      if (dtype === 'tostring') {
        if (Array.isArray(obj)) return obj.join('');else if (typeof obj !== 'string') return obj.toString();
      }
    }
      ...
      switch (typeof val) {
        case "string":
          if (limitStrings && val.length > limitStrings) {
            oot[key] = "OMITTED STRING, originally " + val.length + " bytes long";
          } else {
            oot[key] = val;
          }
          break;
        case "object":
          if (val == null || Array.isArray(val) || "toJSON" in val || curDepth >= 2) {
            oot[key] = val;
          } else {
  ...
  function boundedCmpObjs(a, b, depthLeft) {
    ...
    for (key in a) {
      ...
      if (depthLeft) {
        if (!smartCompareEquiv(a[key], b[key], nextDepth)) return false;
      } else {
  ...
  function smartCompareEquiv(a, b, depthLeft) {
    ...
    if (Array.isArray(a)) {
      ...
      for (var iArr = 0; iArr < a.length; iArr++) {
        if (!smartCompareEquiv(a[iArr], b[iArr], depthLeft - 1)) return false;
      }
      return true;
    }
  ...
  LoggestClassMaker.prototype = {
    ...
    _define: function (name, type) {
      if (this._definedAs.hasOwnProperty(name)) {
        throw new Error("Attempt to define '" + name + "' as a " + type + " when it is already defined as a " + this._definedAs[name] + "!");
      }
    ...
    _wrapLogProtoForTest: function (name) {
      ...
      this.testLogProto[name] = function () {
        ...
        var testActor = this._actor;
        if (testActor) testActor.__loggerFired();
        return rval;
      };
    },
    ...
    addStateVar: function (name) {
      ...
      this.testActorProto['expect_' + name] = function (val) {
        ...
        if (!this._ignore || !this._ignore[name]) this._expectations.push([name, gimmeStack(), val]);
        return this;
      };
    ...
    addLatchedState: function (name) {
      ...
      this.testLogProto[name] = this.logProto[name] = this.dummyProto[name] = function (val) {
    ...
    addEvent: function (name, args, testOnlyLogArgs) {
      ...
      this.testActorProto['expect_' + name] = function () {
        ...
        // If we are being told to ignore stuff this round, eat the expectation.
        if (!this._ignore || !this._ignore[name]) this._expectations.push(exp);
        return this;
      };
      this.testActorProto['ignore_' + name] = makeIgnoreFunc(name);
      this.testActorProto['_verify_' + name] = function (tupe, entry) {
        ...
        for (var iArg = 2; iArg < tupe.length; iArg++) {
          if (!smartCompareEquiv(tupe[iArg], entry[iArg - 1], COMPARE_DEPTH)) return false;
        }
        return true;
      };
    },
    addAsyncJob: function (name, args, testOnlyLogArgs) {
      var name_begin = name + '_begin',
          name_end = name + '_end';
      ...
      this.testActorProto['expect_' + name_begin] = function () {
        ...
        for (var iArg = 0; iArg < arguments.length; iArg++) {
          if (useArgs[iArg] && useArgs[iArg] !== EXCEPTION) exp.push(arguments[iArg]);
        }
        // If we are being told to ignore stuff this round, eat the expectation.
        if (!this._ignore || !this._ignore[name_begin]) this._expectations.push(exp);
        return this;
      };
      ...
      this.testActorProto['expect_' + name_end] = function () {
        ...
        // If we are being told to ignore stuff this round, eat the expectation.
        if (!this._ignore || !this._ignore[name_end]) this._expectations.push(exp);
        return this;
      };
      this.testActorProto['ignore_' + name_end] = makeIgnoreFunc(name_end);
      this.testActorProto['_verify_' + name_begin] = this.testActorProto['_verify_' + name_end] = function (tupe, entry) {
        // only check arguments we had expectations for.
        for (var iArg = 2; iArg < tupe.length; iArg++) {
          if (!smartCompareEquiv(tupe[iArg], entry[iArg - 1], COMPARE_DEPTH)) return false;
        }
    ...
    addCall: function (name, logArgs, testOnlyLogArgs) {
      ...
      this.dummyProto[name] = function () {
        var rval;
        try {
          rval = arguments[numLogArgs + 1].apply(arguments[numLogArgs], Array.prototype.slice.call(arguments, numLogArgs + 2));
        } catch (ex) {
      ...
      this.logProto[name] = function () {
        ...
        this._entries.push(entry);
        try {
          rval = arguments[numLogArgs + 1].apply(arguments[numLogArgs], Array.prototype.slice.call(arguments, iArg + 2));
          entry.push($microtime.now());
          entry.push(gSeq++);
          entry.push(null);
        } catch (ex) {
      ...
      this.testActorProto['expect_' + name] = function () {
        ...
        var exp = [name, gimmeStack()];
        for (var iArg = 0; iArg < arguments.length; iArg++) {
          if (useArgs[iArg]) exp.push(arguments[iArg]);
        }
        // If we are being told to ignore stuff this round, eat the expectation.
        if (!this._ignore || !this._ignore[name]) this._expectations.push(exp);
        return this;
    ...
    makeFabs: function () {
       ...
       */
      var loggerDecisionFab = function loggerDecisionFab(implInstance, parentLogger, ident) {
        var logger, tester;
        // - Testing
        if (tester = moduleFab._underTest || loggerDecisionFab._underTest) {
          ...
          if (typeof parentLogger === "string") throw new Error("A string can't be a logger => not a valid parent");
        ...
        if (parentLogger) {
          if (parentLogger._kids === undefined) {} else if (parentLogger._kids === null) {
  ...
  var LEGAL_FABDEF_KEYS = ['implClass', 'type', 'subtype', 'topBilling', 'semanticIdent', 'dicing', 'stateVars', 'latchState', 'events', 'asyncJobs', 'calls', 'errors', 'TEST_ONLY_calls', 'TEST_ONLY_events', 'TEST_ONLY_asyncJobs', 'LAYER_MAPPING'];

  function augmentFab(mod, fab, defs) {
    var testActors = fab._testActors,
        rawDefs = fab._rawDefs;

    for (var defName in defs) {
      var key,
          loggerDef = defs[defName],
          testOnlyMeta;
      ...
      if ("events" in loggerDef) {
        ...
        if ("TEST_ONLY_events" in loggerDef) testOnlyEventsDef = loggerDef.TEST_ONLY_events;
        for (key in loggerDef.events) {
          testOnlyMeta = null;
          if (testOnlyEventsDef && testOnlyEventsDef.hasOwnProperty(key)) testOnlyMeta = testOnlyEventsDef[key];
          maker.addEvent(key, loggerDef.events[key], testOnlyMeta);
        }
      }
      if ("asyncJobs" in loggerDef) {
        var testOnlyAsyncJobsDef = null;
        if ("TEST_ONLY_asyncJobs" in loggerDef) testOnlyAsyncJobsDef = loggerDef.TEST_ONLY_asyncJobs;
        for (key in loggerDef.asyncJobs) {
          testOnlyMeta = null;
          if (testOnlyAsyncJobsDef && testOnlyAsyncJobsDef.hasOwnProperty(key)) testOnlyMeta = testOnlyAsyncJobsDef[key];
          maker.addAsyncJob(key, loggerDef.asyncJobs[key], testOnlyMeta);
        }
      }
      if ("calls" in loggerDef) {
        var testOnlyCallsDef = null;
        if ("TEST_ONLY_calls" in loggerDef) testOnlyCallsDef = loggerDef.TEST_ONLY_calls;
        for (key in loggerDef.calls) {
          testOnlyMeta = null;
          if (testOnlyCallsDef && testOnlyCallsDef.hasOwnProperty(key)) testOnlyMeta = testOnlyCallsDef[key];
      ...
      reportNewLogger: function (logger, parentLogger) {
        logger._actor = {
          __loggerFired: function () {
            ...
            dumpFunc(logger.__defName + '(' + logger._ident + ')' + JSON.stringify(entry) + '\n');
          }
        };
        return parentLogger;
      }

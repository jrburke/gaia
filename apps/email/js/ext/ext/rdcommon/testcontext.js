...
define(['./log', 'exports'], function ($log, exports) {
  ...
  function TestStep(_log, kind, descBits, actors, testFunc, isBoring, groupName) {
    ...
    if (groupName) this.log.group(groupName);
  }
  TestStep.prototype = {
    toString: function () {
      return '[TestStep]';
  ...
  function TestContext(testCase, permutationIndex) {
    ...
    this._log = LOGFAB.testCasePermutation(this, testCase.log, permutationIndex);
  ...
  TestContext.prototype = {
     ...
     */
    _mixinFromHelperDefs: function (target, what, type, invokeConstructor, constructorArgs) {
      ...
      if (helperDefs) {
        for (var iHelpDef = 0; iHelpDef < helperDefs.length; iHelpDef++) {
          ...
          if (!(useDict in helperDef) || !helperDef[useDict].hasOwnProperty(type)) continue;
        ...
        if (invokeConstructor && '__constructor' in target) target.__constructor.apply(target, constructorArgs);
      }
    },
    ...
    actor: function actor(type, name, opts, optionalParentActor) {
      ...
      for (var iFab = 0; iFab < fabs.length; iFab++) {
        ...
        if (actorDir.hasOwnProperty(type)) {
          // - create the actor
          var actor = new actorDir[type](name, optionalParentActor ? optionalParentActor._uniqueName : null);
          ...
          if ("__constructor" in actor) {
            this._log.actorConstructor(type, name, actor, actor.__constructor, actor, opts);
          }
    ...
    ownedThing: function ownedThing(actor, type, humanName, digitalName) {
      var thang = $log.__makeThing(type, humanName, digitalName);
      this._mixinFromHelperDefs(thang, 'thing', type, true, []);
      if (!actor._logger._named) actor._logger._named = {};
      ...
      for (iArg = 0; iArg < args.length - 1; iArg++) {
        ...
        if (Array.isArray(arg)) {
          for (var iNestedArg = 0; iNestedArg < arg.length; iNestedArg++) {
            ...
            if ($log.TestActorProtoBase.isPrototypeOf(nestedArg)) actors.push(nestedArg);
            descBits.push(nestedArg);
          }
        } else {
          if ($log.TestActorProtoBase.isPrototypeOf(arg)) actors.push(arg);
          descBits.push(arg);
        }
      }
      var testFunc = args[iArg];
      var step = new TestStep(this._log, kind, descBits, actors, testFunc, isBoring, this._definingGroup);
      this.__steps.push(step);
      return step;
    },

    _newDeferredStep: function (kind, args, isBoring) {
      if (!this._deferredSteps) this._deferredSteps = [];
    ...
    permutation: function permutation(variesDesc, variants) {
      ...
      for (var i = 0; i < numVariants.length; i++) {
        if (variants[i] !== this.__steps[baseStep]) throw new Error("Step sequence invariant violation");
      }
  ...
  exports.defineTestsFor = function defineTestsFor(testModule, logfabs, testHelpers, tags) {
    if (logfabs == null) logfabs = [];else if (!Array.isArray(logfabs)) logfabs = [logfabs];else // need to be able to mutate it
      logfabs = logfabs.concat();
    if (testHelpers == null) testHelpers = [];else if (!Array.isArray(testHelpers)) testHelpers = [testHelpers];
    ...
    for (var iHelper = 0; iHelper < testHelpers.length; iHelper++) {
      ...
      if ("LOGFAB_DEPS" in testHelper) {
        ...
        for (var iFab = 0; iFab < testHelper.LOGFAB_DEPS.length; iFab++) {
          ...
          if (logfabs.indexOf(logfab) === -1) logfabs.push(logfab);
        }
      }
      ...
      if ('TESTHELPER_DEPS' in testHelper) {
        for (var iSub = 0; iSub < testHelper.TESTHELPER_DEPS.length; iSub++) {
          ...
          if (testHelpers.indexOf(subHelper) === -1) testHelpers.push(subHelper);
        }
      }
    }
    ...
    reportNewLogger: function (logger, currentParent) {
      if (gNextLazyLoggerActor) {
        ...
        if (!currentParent && gNextLazyLoggerActor.RT._loggerStack.length) {
          currentParent = gNextLazyLoggerActor.RT._loggerStack[gNextLazyLoggerActor.RT._loggerStack.length - 1];
        }

...
define(['./deferred', './testcontext', './extransform', 'require', 'exports'], function (Deferred, $testcontext, $extransform, require, exports) {
  ...
  TestRuntimeContext.prototype = {
    ...
    popLogger: function (logger) {
      ...
      if (idx !== -1) this._loggerStack.splice(idx, 1);
    },
    ...
    reportPendingActor: function (actor) {
      ...
      if (!this._pendingActorsByLoggerType.hasOwnProperty(type)) this._pendingActorsByLoggerType[type] = [actor];else this._pendingActorsByLoggerType[type].push(actor);
    },
    ...
    captureAllLoggersByType: function (type, dict) {
      if (dict) this._captureAllLoggersByType[type] = dict;else delete this._captureAllLoggersByType[type];
    },
    ...
    reportNewLogger: function (logger, curParentLogger) {
      ...
      if (this._pendingActorsByLoggerType.hasOwnProperty(type) && this._pendingActorsByLoggerType[type].length) {
      ...
      } else if (this._captureAllLoggersByType.hasOwnProperty(type) && typeof logger._ident !== 'object') {
          this._captureAllLoggersByType[type][logger._ident] = logger;
        }

      // - if there is no explicit parent, use the top of the logger stack
      if (!curParentLogger && this._loggerStack.length) return this._loggerStack[this._loggerStack.length - 1];
    ...
    reportActiveActorThisStep: function (actor) {
      if (!actor) throw new Error("You are passing in a null actor!");
      if (this._liveActors === null) throw new Error("We are not in a step!");
      if (actor._activeForTestStep) return;
      this._liveActors.push(actor);
      actor.__prepForTestStep(this);
    },

    peekLogger: function () {
      if (this._loggerStack.length) return this._loggerStack[this._loggerStack.length - 1];
      return null;
    }
   ...
   */
  function TestDefinerRunner(testDefiner, superDebug, exposeToTestOptions, resultsReporter) {
    if (!testDefiner) throw new Error("No test definer provided!");
  ...
  TestDefinerRunner.prototype = {
    ...
    runTestStep: function (step) {
      ...
      if (superDebug) superDebug("====== Running Step: " + step.log._ident);
      var iActor, actor;
      ...
      if (rval instanceof Error) {
        if (superDebug) superDebug(" :( encountered an error in the step func:", rval);
        step.log.run_end();
      ...
      if (superDebug) superDebug(" there are", liveActors.length, "live actors this step", "up from", step.actors.length, "step-defined actors");
      var promises = [],
          allGood = true;
      for (iActor = 0; iActor < liveActors.length; iActor++) {
        ...
        if (waitVal.then) {
          ...
          if (superDebug) superDebug(" actor", actor.__defName, actor.__name, "generated a promise");
        }
        // if it's not a promise, it must be a boolean
        else if (!waitVal) {
            if (superDebug) {
              var whySad;
              if (actor._expectNothing && (actor._expectations.length || actor._iExpectation)) whySad = 'expected nothing, got something';else if (!actor._expectationsMetSoFar) whySad = 'expectations not met after ' + actor._iExpectation;else whySad = 'unsure';
              superDebug(" :( waitVal synchronously resolved to false on " + actor + " because: " + whySad);
            }
            allGood = false;
          }
      }
      ...
      if (!promises.length) {
        ...
        function failStep() {
          ...
          for (var iActor = 0; iActor < liveActors.length; iActor++) {
            actor = liveActors[iActor];
            if (!actor._logger) step.log.actorNeverGotLogger(actor.__defName, actor.__name);else actor.__failUnmetExpectations();
            actor.__resetExpectations();
          }
          ...
          if (superDebug) superDebug(' :( timeout, fail');
          step.log.timeout();
          step.log.result('fail');
        ...
        var countdownTimer = setTimeout(function () {
          if (self._superDebug) self._superDebug("!! timeout fired, deferred?", deferred !== null);
          if (!deferred) return;
          failStep();
        }, step.timeoutMS || DEFAULT_STEP_TIMEOUT_MS);
        // -- promise resolution/rejection handler
        if (this._superDebug) this._superDebug("waiting on", promises.length, "promises");
        Promise.all(promises).then(function passed() {
          if (self._superDebug) self._superDebug("!! all resolved, deferred?", deferred !== null);
          if (!deferred) return;
          ...
          for (var iActor = 0; iActor < liveActors.length; iActor++) {
            ...
            if (!actor.__resetExpectations()) {
              passed = false;
              if (superDebug) superDebug(' :( weird actor error on: ' + actor);
            }
          }
          self._runtimeContext._liveActors = null;
          ...
          if (self._superDebug) self._superDebug("!! failed, deferred?", deferred !== null);
          if (!deferred) return;
    ...
    runTestCasePermutation: function (testCase, permutationNum) {
      ...
      return new Promise((function (resolve, reject) {
        ...
        var rval = defContext._log.setupFunc({}, testCase.setupFunc, defContext, self._runtimeContext);
        if (rval instanceof Error) {
          // in the event we threw during the case setup phase, it's a failure.
          if (self._superDebug) self._superDebug(' :( setup func error thrown! ' + rval);
        ...
        function runNextStep(passed) {
          ...
          if (iStep >= defContext.__steps.length) {
            ...
            if (self._superDebug) self._superDebug("========= Done Case: " + testCase.desc + "\n");
            // - resolve!
          ...
          if (runIt) self.runTestStep(step).then(runNextStep);else // for stack simplicity, run the skip in a when, but not required.
    ...
    runTestCase: function (testCase) {
      ...
      this._runtimeContext = new TestRuntimeContext(this._exposeToTestOptions, this._fileBlackboard);
    ...
    runAll: function (errorTrapper, overrideStepDuration) {
      if (overrideStepDuration) DEFAULT_STEP_TIMEOUT_MS = overrideStepDuration;

      //console.error(" runAll()");
      var deferred = new Deferred("TestDefinerRunner.runAll"),
          iTestCase = 0,
          definer = this._testDefiner,
          self = this;
      ...
      function runNextTestCase() {
        ...
        if (iTestCase >= definer.__testCases.length) {
          ...
          errorTrapper.removeListener('uncaughtException', uncaughtExceptionHandler);
      ...
      function uncaughtExceptionHandler(ex) {
        if (self._logBadThingsToLogger) self._logBadThingsToLogger.uncaughtException(ex);
      }
  ...
  function detectAndReportJsonCycles(obj) {
    ...
    function recurse(what) {
      if (what == null || typeof what !== 'object') return;
      ...
      var use;
      if ("toJSON" in what) use = what.toJSON();else use = what;

      for (var key in use) {
        // JSON traversal is shallow; nb: we could use ES5 instead of this hack
        if (!use.hasOwnProperty(key)) continue;
        var val = use[key];
   ...
   */
  function reportTestModuleRequireFailures(testModuleName, moduleName, variant, exceptions, resultsReporter) {
    var dumpObj = {
   ...
   */
  exports.runTestsFromModule = function runTestsFromModule(testModuleName, runOptions, ErrorTrapper, superDebug) {
    var deferred = new Deferred("runTestsFromModule:" + testModuleName);
    var runner;
    function itAllGood() {
      if (superDebug) superDebug('All tests in "' + testModuleName + '" run, ' + 'generating results.');
      runner.reportResults();
      deferred.resolve(true);
    };

    var resultsReporter = runOptions.resultsReporter || makeStreamResultsReporter(ErrorTrapper.reliableOutput);

    var variant = null;
    if (runOptions && runOptions.variant) variant = runOptions.variant;
    ...
    ErrorTrapper.callbackOnError(function explodey(err, moduleName) {
      ...
      if (alreadyBailed) return;
      reportTestModuleRequireFailures(testModuleName, moduleName, variant, [err], resultsReporter);
      deferred.resolve(true);
      alreadyBailed = true;
    ...
    require([testModuleName], function (tmod) {
      ...
      if (trappedErrors.length) {
        reportTestModuleRequireFailures(testModuleName, '', variant, trappedErrors, resultsReporter);
        deferred.resolve(true);
        return;
      }
      if (!tmod.TD) {
        var fakeError = new Error("Test module: '" + testModuleName + "' does not export a 'TD' symbol!");
        reportTestModuleRequireFailures(testModuleName, testModuleName, variant, [fakeError], resultsReporter);
        deferred.resolve(true);
        return;
      }

      // now that it is loaded, run it
      if (runOptions.hasOwnProperty('defaultStepDuration')) DEFAULT_STEP_TIMEOUT_MS = runOptions.defaultStepDuration;
      runner = new TestDefinerRunner(tmod.TD, superDebug, runOptions.exposeToTest, resultsReporter);

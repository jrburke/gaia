...
define(['./main-router', './configparser-main', './cronsync-main', './devicestorage-main', './maildb-main', './net-main', '../mailapi', 'exports'], function ($router, $configparser, $cronsync, $devicestorage, $maildb, $net, $mailapi, exports) {
    ...
    process: function (uid, cmd, args) {
       ...
       */
          ...
          try {
            var func;
            eval('func = ' + args.func + ';');
            gMailAPI._getBodyForMessage({ id: args.headerId, date: args.headerDate }, null, function (body) {
         ...
         */
            ...
            tablesAndKeyPrefixes.forEach(function (checkArgs) {
              var store = trans.objectStore(checkArgs.table),
                  range = IDBKeyRange.bound(checkArgs.prefix, checkArgs.prefix + '\ufff0', false, false),
                  req = store.get(range);
              req.onerror = function (event) {
                results.push({ errCode: event.target.errorCode });
                if (--waitCount === 0) sendResults();
              };
  ...
  var mainBridge = {
    ...
    sendMessage: null,
    process: function (uid, cmd, args) {
      if (gMailAPI) gMailAPI.__bridgeReceive(args);
    }
  };
  ...
  var deviceStorageTestHelper = {
    ...
    process: function (uid, cmd, args) {
      if (cmd === 'attach') {
        ...
        req.onsuccess = (function () {
          this.sendMessage(null, 'got', { id: args.id, error: null, blob: req.result });
        }).bind(this);
        req.onerror = (function () {
          this.sendMessage(null, 'got', { id: args.id, error: '' + req.error, blob: null });
        }).bind(this);
      }
    },
    _storage: null,

define(function (require, exports) {
  ...
  function MimeHeaderInfo(rawHeaders, opts) {
    ...
    this.contentType = this.getParameterHeader('content-type') || 'application/octet-stream';
    [this.mediatype, this.subtype] = this.contentType.split('/');
    this.contentId = util.stripArrows(this.getStringHeader('content-id'));
    this.filename = this.getParameterHeader('content-type', 'name') || this.getParameterHeader('content-disposition', 'filename');
    ...
    this.references = extractReferences(this.getStringHeader('references'), this.guid);

    this.author = this.getAddressHeader('from', [])[0] || { name: 'Missing Author', address: 'missing@example.com' };
  ...
  MimeHeaderInfo.prototype = {
    ...
    getParameterHeader(headerName, /* optional */paramName) {
      ...
      if (value) {
        var params = jsmime.headerparser.parseParameterHeader(jsmime.headerparser.decodeRFC2047Words(value),
    ...
    getAddressHeader(headerName, defaultValue) {
      var allResults = (this.rawHeaders[headerName] || []).reduce(function (results, header) {
        return results.concat(jsmime.headerparser.parseAddressingHeader(header, /* doRFC2047: */true).map(function (addr) {
          if (addr.group) {
    ...
    _computePracticalDisposition() {
      ...
      if (disposition) {
        ...
        if (disposition.toLowerCase() === 'inline' && this.mediatype !== 'text' && !this.contentId) {
      ...
      else if (this.parentContentType === 'multipart/related' && this.contentId && this.mediatype === 'image') {
    ...
    var parser = new jsmime.MimeParser({
      ...
      startPart(jsmimePartNum, jsmimeHeaders) {
        ...
        var headers = MimeHeaderInfo.fromJSMime(jsmimeHeaders, {
          parentContentType: partNum.indexOf('.') !== -1 ? partToContentTypeMap.get(partNum.slice(0, partNum.lastIndexOf('.'))) : null
        });
  ...
  exports.readAttachmentStreamWithChunkFlushing = co.wrap(function* (contentType, bodyStream, flushFileFn) {
    ...
    for (;;) {
      ...
      if (!done) {
        ...
        console.warn(`All blobs fetched.` + `Concatenating into a ${ contentType } super-blob...`);

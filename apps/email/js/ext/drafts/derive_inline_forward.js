define(function (require) {
  ...
  const { addressPairFromIdentity, replyToFromIdentity } = require('./address_helpers');

  const { generateForwardSubject, generateForwardParts } = require('../bodies/mailchew');
  ...
  return function* deriveInlineForward({ sourceMessage, identity, messageId, umid,
    guid, date, folderIds }) {
    // -- Subject
    var subject = generateForwardSubject(sourceMessage.subject);

    // -- Build the body
    var bodyReps = yield* generateForwardParts(sourceMessage, identity);

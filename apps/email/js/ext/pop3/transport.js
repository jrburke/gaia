define(function (require, exports) {
  ...
  exports.Pop3RequestStream = function (socket, greetingRequest) {
    ...
    readable.pipeThrough(new mimeStreams.LineTransformStream()).pipeTo(new streams.WritableStream({
      write: function (line) {
  ...
  var Request = exports.Request = function (command, args, expectMultiline) {
    ...
    this.dataLineStream = new streams.ReadableStream({
      start: function (controller) {
        _this._dataLineStreamController = controller;
      }
    });
  };
  ...
  Request.prototype = {
    ...
    then: function (thenFn, catchFn) {
      return mimeStreams.readAllChunks(this.dataLineStream).then(thenFn, catchFn);
    },

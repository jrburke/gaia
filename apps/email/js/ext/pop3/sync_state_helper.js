define(function (require) {
  ...
  SyncStateHelper.prototype = {
    ...
    issueUniqueMessageId: function () {
      return this._accountId + '.' + a64.encodeInt(this.rawSyncState.nextUmidSuffix++);
    },
    ...
    deltaCheckUidls: function (allMessages) {
      var uidlToUmid = this._uidlToUmid;
      var deletedUidls = this._deletedUidls;
      var overflowUidlsToSize = this._overflowUidlsToSize;
      ...
      var unseenSynced = new Set(uidlToUmid.keys());
      var unseenDeleted = new Set(deletedUidls);
      var unseenOverflow = new Set(overflowUidlsToSize.keys());

      var newMessageBudget = this.maxNewMessages;

      for (var { uidl, size } of allMessages) {
        // uidl, size, number
      ...
      for (var uidl of unseenDeleted) {
        deletedUidls.delete(uidl);
      }
      for (var uidl of unseenOverflow) {
    ...
    newMessageToSync: function (uidl, size) {
      var umid = this.issueUniqueMessageId();
    ...
    cleanupUnseenSyncedMessages: function (unseenSynced) {
      for (var uidl of unseenSynced) {
        var umid = this._uidlToUmid.get(uidl);
    ...
    syncOverflowMessages: function (count) {
      var uidlsToSync = Array.from(this._overflowUidlsToSize.keys()).slice(0, count);
      for (var uidl of uidlsToSync) {
    ...
    inferDeletionFromExistingUids: function (newUids) {
      var uidsNotFound = new Set(this._uidInfo.keys());
      for (var uid of newUids) {
        uidsNotFound.delete(uid);
      }

      var { _uidInfo: uidInfo, umidDeletions, umidNameReads,
        umidLocationWrites } = this;
      for (var uid of uidsNotFound) {
        var { umid } = uidInfo.get(uid);
        uidInfo.delete(uid);

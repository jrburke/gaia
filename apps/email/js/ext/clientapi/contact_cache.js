define(function (require) {
  ...
  var ContactCache = {
    ...
    shoddyAutocomplete: function (phrase) {
      ...
      for (var peeps of this._livePeepsByEmail.values()) {
    ...
    _onContactChange: function (event) {
      ...
      if (event.reason === 'remove') {
        ...
        if (!event.contactID) {
          for (var livePeeps of livePeepsById.values()) {
        ...
        // - just one contact removed
            ...
            var livePeeps = livePeepsById.get(event.contactID);
            if (livePeeps) {
              cleanOutPeeps(livePeeps);
      ...
      // -- Created or updated; we need to fetch the contact to investigate
          ...
          req.onsuccess = function () {
            ...
            if (event.reason === 'update') {
              var livePeeps = livePeepsById.get(contact.id);
              if (livePeeps) {
            ...
            for (var iEmail = 0; iEmail < contact.email.length; iEmail++) {
              ...
              var livePeeps = livePeepsByEmail.get(email);
            ...
            var self = this,
              ...
              if (req.result && req.result.length) {
                ...
                for (var i = 0; i < peepsToFixup.length; i++) {
                  var fixupPeep = peepsToFixup[i];
                  if (!fixupPeep.contactId) {
                    fixupPeep.contactId = contact.id;
                    var _livePeeps = self._livePeepsById.get(fixupPeep.contactId);
                    if (_livePeeps === undefined) {
                      _livePeeps = [];
                      self._livePeepsById.set(fixupPeep.contactId, _livePeeps);
                    }
                    _livePeeps.push(fixupPeep);
                  }
              ...
              if (--self.pendingLookupCount === 0) {
                for (var i = 0; i < ContactCache.callbacks.length; i++) {

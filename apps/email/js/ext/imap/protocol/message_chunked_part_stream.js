define(function (require) {
  ...
  return function messageChunkedPartStream({
    ...
    var remainingPartsToFetch = parts.slice();
    ...
    return new ReadableStream({
      ...
      pull: co.wrap(function* (out) {
        ...
        var blobIndex = 0;
        var partInfo = remainingPartsToFetch.shift();
        var mimeStream = chunkedDownloadMimeStream({
          ctx,
          pimap,
          folderInfo,
          uid,
        ...
        var mimeReader = mimeStream.getReader();

        // (We do not need the headers since it's information sourced from the
        // partInfo and already known to our caller.)
        var { value: { bodyStream } } = yield mimeReader.read();
        var bodyReader = bodyStream.getReader();

        for (;;) {
          var { value: blob, done } = yield bodyReader.read();
          if (!done) {
            out.enqueue({

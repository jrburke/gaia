define(function (require) {
   ...
   *   tradeoffs.
   */
  return function chunkedDownloadMimeStream({ ctx, pimap, folderInfo, uid, partInfo, downloadChunkSize, saveChunkSize }) {
    var byteIndex = 0;

    var byteStream = new ReadableStream({
      start(out) {
        out.enqueue(mimefuncs.charset.encode('Content-Type: ' + partInfo.type + '\r\n' + 'Content-Transfer-Encoding: ' + partInfo.encoding + '\r\n' + '\r\n'));
      },
      pull(out) {
        return pimap.fetchBody(ctx, folderInfo, {
          uid,
          part: partInfo.part,
          byteRange: {
            offset: byteIndex,
    ...
    return byteStream.pipeThrough(new MimeNodeTransformStream({ saveChunkSize, mimeType: partInfo.type }));
  };
});

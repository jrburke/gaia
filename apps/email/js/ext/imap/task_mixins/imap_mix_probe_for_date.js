define(function (require) {
  ...
  const { INITIAL_SYNC_GROWTH_DAYS, GROWTH_MESSAGE_COUNT_TARGET } = require('../../syncbase');
  ...
  return {
    ...
    _probeForDateUsingSequenceNumbers: co.wrap(function* ({
      ...
      var probeStep = Math.ceil(GROWTH_MESSAGE_COUNT_TARGET / 4);
      ...
      var seqs = [];
      for (var curSeq = startSeq; curSeq >= 1; curSeq -= probeStep, probeStep *= PROBE_STEP_SCALE) {
        seqs.push(curSeq);
      }

      var { result: messages } = yield account.pimap.listMessages(ctx, folderInfo, seqs, ['INTERNALDATE'], {});
      ...
      var violationsDelta = 0;
      var checkDate = 0;
      for (var msg of messages) {
        var msgDate = parseImapDateTime(msg.internaldate);
        if (!checkDate) {
          checkDate = msgDate;
      ...
      if (violationsDelta > 7 * DAY_MILLIS) {
        logic(ctx, 'dateProbeBail', { violationDays: Math.floor(violationsDelta / DAY_MILLIS) });
      ...
      var useDate = quantizeDate(parseImapDateTime(messages[Math.min(messages.length - 1, 2)].internaldate));

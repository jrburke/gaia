define(function (require) {
  ...
  var { convIdFromMessageId } = require('../../id_conversions');
  ...
  FolderSyncStateHelper.prototype = {
    ...
    issueUniqueMessageId: function () {
      return this._folderId + '.' + a64.encodeInt(this.rawSyncState.nextUmidSuffix++);
    },
    ...
    _findFlagSlot: function (flags) {
      flags.sort();
      var normStr = JSON.stringify(flags);
      var idx = this._flagSets.indexOf(normStr);
      if (idx === -1) {
    ...
    _allocateFlagSlot: function (normStr) {
      var idx = this._flagSets.indexOf(null);
      if (idx === -1) {
    ...
    _decrFlagSlot: function (slot) {
      var newVal = --this._flagSetCounts[slot];
      if (newVal === 0) {
    ...
    getUmidForUid: function (uid) {
      var info = this._uidInfo.get(uid);
      if (info) {
        return info.umid;
      } else {
        return null;
    ...
    yayMessageFoundByDate: function (uid, dateTS, flags) {
      var flagSlot = this._findFlagSlot(flags);
    ...
    checkFlagChanges: function (uid, flags) {
      var info = this._uidInfo.get(uid);
      var oldFlagSlot = info.flagSlot;
      var newFlagSlot = this._findFlagSlot(flags);
    ...
    generateSyncConvTasks: function () {
      var umidNameReads = this.umidNameReads;
      for (var [umid, flags] of this.umidFlagChanges) {
        var messageId = umidNameReads.get(umid);
        if (messageId) {
          this._ensureTaskWithUmidFlags(messageId, umid, flags);
        }
      }
      for (var umid of this.umidDeletions) {
        var messageId = umidNameReads.get(umid);
        if (messageId) {
    ...
    _ensureTaskWithUmidFlags: function (messageId, umid, flags) {
      ...
      var task = this._ensureConvTask(convId);

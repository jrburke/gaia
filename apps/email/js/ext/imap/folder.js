define(['logic', '../a64', '../allback', '../date', '../syncbase', '../util', 'module', 'require', 'exports'], function (logic, $a64, $allback, $date, $sync, $util, $module, require, exports) {
  ...
  function compactArray(arr) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      if (delta) arr[i - delta] = obj;
    }
    if (delta) arr.splice(len - delta, delta);
    return arr;
  }
  ...
  ImapFolderConn.prototype = {
    ...
    _timelySyncSearch: function (searchOptions, searchedCallback, abortedCallback, progressCallback, isRetry) {
      ...
      if (!this._conn) {
        ...
        this.acquireConn(this._timelySyncSearch.bind(this, searchOptions, searchedCallback, abortedCallback, progressCallback,
        /* isRetry: */isRetry), abortedCallback, 'sync', true);
        return;
      }
          ...
          if (!isRetry) {
            ...
            abortedCallback = (function () {
              ...
              if (!gotSearchResponse) {
                ...
                this._timelySyncSearch(searchOptions, searchedCallback, origAbortedCallback, progressCallback,
                /* isRetry: */true);
              }
    ...
    _lazySyncDateRange: function (startTS, endTS, accuracyStamp, doneCallback, progressCallback) {
      ...
      Promise.all([imapSearchPromise, databaseFetchPromise]).then((function (results) {
        ...
        uidSet.forEach(function (uid) {
          ...
          // Updated
              ...
              if (localHeader.imapMissingInSyncRange) {
                ...
                storage.updateMessageHeader(localHeader.date, localHeader.id, true, localHeader,
            ...
            // Deleted or Ambiguously Deleted
                ...
                localHeader.imapMissingInSyncRange.startTS > endTS)) {
                  ...
                  // intact.)
                  missingRange = localHeader.imapMissingInSyncRange = { startTS: startTS || 0, endTS: endTS || Infinity };
                } else {
                  // (#3, union!)
                ...
                if (missingRange.startTS <= date - fuzz && missingRange.endTS >= date + fuzz) {
                  slog.log('imap:unambiguously-deleted-uid', { uid: uid, missingRange: missingRange });
                ...
                // later. We've already marked the updated "missing" range above.
                    ...
                    slog.log('imap:ambiguously-missing-uid', { uid: uid, missingRange: missingRange,
        ...
        latch.then(function () {
          ...
          uidSync.oncomplete = function (newCount, knownCount) {
            self._LOG.syncDateRange_end(newCount, knownCount, numDeleted, startTS, endTS);
    ...
    downloadBodyReps: function () {
      ...
      require(['./imapchew', './protocol/bodyfetcher', './protocol/textparser', './protocol/snippetparser'], function (_imapchew, _bodyfetcher, _textparser, _snippetparser) {
    ...
    _lazyDownloadBodyReps: function (header, options, callback) {
      ...
      var gotBody = function gotBody(bodyInfo) {
        ...
        bodyInfo.bodyReps.forEach(function (rep, idx) {
          ...
          if (bytesToFetch <= 0) bytesToFetch = 64;
    ...
    _handleBodyFetcher: function (fetcher, header, body, bodyUpdatedCallback) {
      ...
      fetcher.onparsed = (function (err, req, resp) {
        ...
        this._storage.updateMessageHeader(header.date, header.id, false, header, body, req.headerUpdatedCallback.bind(null, null) // no error
        );
      ...
      fetcher.onend = (function () {
        ...
        this._storage.updateMessageBody(header, body, {}, event, bodyUpdatedCallback.bind(null, null) // we do not/cannot error
        );
      }).bind(this);
    },
    ...
    downloadBodies: function () {
      ...
      require(['./imapchew', './protocol/bodyfetcher', './protocol/snippetparser'], function (_imapchew, _bodyfetcher, _snippetparser) {

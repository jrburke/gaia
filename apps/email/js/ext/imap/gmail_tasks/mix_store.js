define(function (require) {
  ...
  var GmailStoreTaskMixin = {
    ...
    deriveMemoryStateFromPersistentState: function (persistentState, accountId) {
      var markers = [];
      var idToAggrString = new Map();

      for (var [aggrString, change] of persistentState.aggrChanges) {
        for (var uid of change.uids) {
    ...
    plan: co.wrap(function* (ctx, persistentState, memoryState, req) {
      var { aggrChanges } = persistentState;
      var { idToAggrString } = memoryState;

      // (only needed in the labels case currently, but )
      var normalizeHelper = yield this.prepNormalizationLogic(ctx, req.accountId);
      ...
      var modifyTaskMarkers = new Map();
      var anyMessageChanged = false;

      // - Apply the message selector if applicable
      var filteredMessages = selectMessages(loadedMessages, req.onlyMessages, req.messageSelector);
      ...
      for (var message of filteredMessages) {
        var { add: actuallyAdded, remove: actuallyRemoved } = normalizeAndApplyChanges(message[attrName], req.add, req.remove);
        ...
        if (actuallyAdded || actuallyRemoved) {
          ...
          // in the flags case.
          actuallyAdded = this.normalizeLocalToServer(normalizeHelper, actuallyAdded);
          actuallyRemoved = this.normalizeLocalToServer(normalizeHelper, actuallyRemoved);

          modifiedMessagesMap.set(message.id, message);
          anyMessageChanged = true;

          var uid = numericUidFromMessageId(message.id);
          ...
          if (idToAggrString.has(uid)) {
            var pendingAggrString = idToAggrString.get(uid);
            var pendingChanges = aggrChanges.get(pendingAggrString);

            ({ add: actuallyAdded, remove: actuallyRemoved } = mergeChanges(pendingChanges, { add: actuallyAdded, remove: actuallyRemoved }));
          ...
          if (actuallyAdded || actuallyRemoved) {
            var newAggrString = this._deriveMixStoreAggrString(actuallyAdded, actuallyRemoved);
            var newChanges = undefined;
    ...
    consult: function (askingCtx, persistentState, memoryState, argDict) {
      var { uid, value } = argDict;

      var { aggrChanges } = persistentState;
      var { idToAggrString } = memoryState;

      if (idToAggrString.has(uid)) {
        var aggrString = idToAggrString.get(uid);
        var changes = aggrChanges.get(aggrString);
    ...
    execute: co.wrap(function* (ctx, persistentState, memoryState, marker) {
      ...
      var changes = aggrChanges.get(marker.aggrString);
